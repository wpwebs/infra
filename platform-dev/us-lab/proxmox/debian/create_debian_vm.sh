# Clone Debian Template and Configure New VM with Separate Boot and Storage Disks
...
# Notes
# * Automation: The script automates the cloning and configuration of a new VM from an Debian template using cloud-init for initial setup and SSH for additional configurations.
...
# This script sets up a new Debian VM by cloning a template and configuring it with specific resources, network settings, and hostname, ensuring a seamless and automated provisioning process.
...
TEMPLATE_ID="9000"        # ID of the Debian template
NEW_VM_ID="9001"          # ID for the new VM
NEW_VM_NAME="debian-clone"
...
VM_IP=$(ssh $PROXMOX_USER@$PROXMOX_HOST "qm guest cmd $NEW_VM_ID network-get-interfaces" | jq -r '.[] | select(.name=="eth0") | ."ip-addresses"[0]."ip-address"')
...
echo "New Debian VM has been created and configured successfully from the template."
